...
if use_default_params:
    MCMC_params = {    
        'MCMC_protocol' : modemc.protocols.pCN_Flip,
        'use_OM' : True,
        ...
        'state0_choice' : 'lower_OM',

        'use_teleport' : True,
        'p_teleport' : 1e-3,
        'teleporter_probs' : [0.02444790377198886, 1-0.02444790377198886],
        'teleporter_choice' : 'OM',
        
        'p_flip' : 1e-2,
        'flip_beta_threshold' : 20,
        
        'enable_parallelisation' : True,
    }

MCMC_params['Nm'] = int(np.round(system_params['Tf'] * MCMC_params['Nm_Tf_factor']))
MCMC_params['system'] = modemc.systems.SwitchNoneq(system_params)
...
def find_instanton(find_upper, use_OM):
    ...
    # Symmetrize Qf to avoid numerical errors
    inst['Qf'] = (inst['Qf'] + inst['Qf'].T)/2
...
def get_shrunken_teleporter(inst, teleporter_Nm):
    ...
    teleporter_Qf = (teleporter_Qf + teleporter_Qf.T)/2
