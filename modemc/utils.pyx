...
cpdef compute_dot_L_xd_x(System system, ts, state, use_OM=True):
    path, d_path = eval_path(system.x0, system.x1, ts, system.Tf, state)
    dim, Nt = path.shape
    ...
    if use_OM:
        system._compute_dot_L_xd_x_OM(ts, path, d_path, dot_L_xd_x)
    else:
        system._compute_dot_L_xd_x_FW(ts, path, d_path, dot_L_xd_x)
