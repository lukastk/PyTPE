...
cpdef compute_Q_matrix(system, instanton_coeffs, band_size, Nm=None, Nq_factor=30, quad_scheme=None, return_partial_Qs=False):
    x0, x1, Tf = system.x0, system.x1, system.Tf
    dim = system.dim
    ...
    Nq = Nm*Nq_factor
    ts, qts, quad_weights = quad_scheme(Nq)
    ts = 0.5*(qts+1)*Tf
    
    L_x_x, L_xd_x, L_xd_xd = utils.compute_hessL(system, ts, instanton_coeffs, use_OM=True)
...
cpdef compute_gelfand_yaglom_determinant(dt, system, state, use_OM, mpmath_dps=500):
    ...
    dot_L_xd_xd = utils.compute_dot_L_xd_x(system, ts, state, use_OM=use_OM)
